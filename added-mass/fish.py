...
class Fish:
    ...
    def __init__(self, config_file):
        ...
        delta_0 = config['thickness_parameter']
        ...
        self.wave_phase = np.deg2rad(config['wave_phase'])
        ...
        self.a_edge_ref = np.pi * self.h_edge_ref * self.w_edge_ref / 4
        self.m_edge_ref = rho * self.a_edge_ref * self.l_edge_ref
        self.I_edge_ref = self.m_edge_ref * (self.w_edge_ref ** 2) / 12
        ...
        self.I_node_ref = (np.concatenate([self.I_edge_ref,[0]]) + np.concatenate([[0], self.I_edge_ref])) / 2
        ...
        self.accelerations = np.zeros((2,self.N))
        ...
        self.CT = self.fluid_density * np.pi * self.CF * (self.h_edge_ref + self.w_edge_ref) * self.l_edge_ref / 8
        self.CN = self.fluid_density * self.CD * self.h_edge_ref * self.l_edge_ref / 2

        # Solve for all BVS values
        self.initialize_bvs()

        # Save control inputs for plotting
    ...
    def heading_controller(self):

        # Compute moment on fish
        midpoints = (self.positions[:, 0:self.N-1] + self.positions[:, 1:self.N]) / 2
        center_of_mass = np.sum(self.m_edge_ref * midpoints, axis=1) / np.sum(self.m_edge_ref)

        # Compute hydrodynamic forces on front half of fish
        front_index = int(np.round(self.N / 2))
        v_ext = (self.external_velocity[:,0:self.N-1] + self.external_velocity[:,1:self.N]) / 2
        v_edge = (self.velocities[:,0:self.N-1] + self.velocities[:,1:self.N]) / 2
        v_rel = v_edge[:, 0:front_index] - v_ext[:, 0:front_index]
        v_edge_T = np.sum(v_rel * self.t[:, 0:front_index], axis=0)
        v_edge_N = np.sum(v_rel * self.n[:, 0:front_index], axis=0)
        forces = (-self.CT[0:front_index] * (v_edge_T * np.abs(v_edge_T)) * self.t[:, 0:front_index]
                  -self.CN[0:front_index] * (v_edge_N * np.abs(v_edge_N)) * self.n[:, 0:front_index])
        
        # Compute angular acceleration from hydrodynamic forces
        moment_arms = midpoints[:, 0:front_index] - center_of_mass[:, np.newaxis]
        fluid_moment = np.sum(forces[0, :] * moment_arms[1, :] - forces[1, :] * moment_arms[0, :])
        fluid_torque = np.sum(self.I_edge_ref + self.m_edge_ref * np.linalg.norm(midpoints - center_of_mass[:, np.newaxis], axis=0) ** 2)
        fluid_angular_acceleration = fluid_moment / fluid_torque
        ...
        current_heading = np.mod(np.arctan2(heading_vec[1], heading_vec[0]), 2*np.pi)
        ...
        if self.time_step < self.period_steps - 1:
            ...
            # Apply control to prevent turning
            offset = 0.5 * np.tanh(fluid_angular_acceleration)
        ...
        if self.time_step >= self.period_steps - 1:
            ...
            # Apply control for turning
            offset = 0.5 * np.tanh(fluid_angular_acceleration + 2 * error_heading)
    ...
    def internal_forces(self, time, z0, u):
        ...
        bending_partials = (0.2 * self.E * self.I_node_ref[1:-1] * (kappa - kappa_ref) / 
        ...
        FHD_edge = (-self.CT * (v_edge_T * np.abs(v_edge_T)) * t
                    -self.CN * (v_edge_N * np.abs(v_edge_N)) * n) # more consistent with high Re swimming
        ...
        mu_normal = self.fluid_density * np.pi * self.CA * self.l_edge_ref * self.h_edge_ref**2 / 4
        ...
        self.accelerations = acceleration.reshape((2,N), order='F')
